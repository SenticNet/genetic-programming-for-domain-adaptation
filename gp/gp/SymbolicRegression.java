...
public class SymbolicRegression
    ...
    public static String fitnessans;
    public static IGPProgram fitnessprog;
    public static double fitnesserror = 1000;
	public static int indx1 = 9;                     
    public static String dirgp = "../neu_gp"+indx1;	
//    public static String dirgp = "neu_gp";
    ...
    public static void main(String[] args)
        ...
        myOutputSolution(fittest, gen);
        ProgramChromosome chrom2 = fittest.getChromosome(0);
        fitnessans = chrom2.toStringNorm(0);
        fitnessprog  = fittest;

        //System.out.println(fitnessans+" iti"); 
        ...
        if (testData != null && testData.length > 0) {
           // System.out.println("\nTesting the fittest program with user defined test data: ");
            ...
            for (int i = 0; i < testDataSize; i++) {
				...
				try{
			    FileWriter ostreamg = new FileWriter(dirgp+"/log_test.txt",true);
                ostreamg.write("testError:"+testResult+"\n");
                ostreamg.close();}
				catch(Exception e){}   
    ...
    public static class FormulaFitnessFunction
        ...
        public double computeRawFitness(final IGPProgram ind) {
            ...
            if(meanError < fitnesserror)fitnesserror = meanError; 
...
try{
                  FileWriter ostreamg = new FileWriter(dirgp+"/log.txt",true);
                      ostreamg.write("meanError:"+appData.getMeanError() +"\n");
                  ostreamg.close();  
                  }catch(Exception e){}

  try{
			    FileWriter ostreamg = new FileWriter(dirgp+"/solution.txt",true);
                ostreamg.write(a_best.toStringNorm(0)+"\n");
                ostreamg.close();}
				catch(Exception e){}   
